...
- **Design tokens live in `src/styles/global.css`** under Tailwind v4's `@theme` block. The palette is a deep-purple-and-black theme: base `--color-bg`/`--color-bg-soft` plus three accent roles `--color-violet` (primary) → `--color-fuchsia` (secondary) → `--color-lavender` (light), consumed via utilities like `text-violet`, `from-violet to-fuchsia`, `bg-lavender/10`. Reusable `@layer components` classes (`.surface`, `.btn`, `.chip`, `.gradient-text`, `.container-x`, `.section`) build on them. Re-theme by editing tokens, not individual components. A few ambient washes use literal `rgba()` in component inline styles (Hero, Contact) — keep those in sync if you change the accents.
